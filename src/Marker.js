...
var Marker = function(lat, lon, text, altitude, previous, scene, _opts){
    ...
    if(!scene._fuse_3d_markerTexture){
        scene._fuse_3d_markerTexture = createMarkerTexture(this.opts.markerColor);
    }

    markerMaterial = new THREE.SpriteMaterial({map: scene._fuse_3d_markerTexture, opacity: .7, depthTest: true, fog: true});
